...
import { IvalidateOwnership } from '../../interfaces/IvalidateContractNFT';

const _0xcertAddress = '0xf176d7bcdD07f8e474877095870685Ef0CCcCb2D';



export async function validateContractNFTAddress(address: string) {
  let result = false;
  try {
    const web3Service = new Web3Service();
    if(!web3Service.web3) return;

    if(!ethers.utils.isAddress(address)) return false;
    const validator = new ERC721Validator(web3Service.web3, _0xcertAddress);
...
export async function validateOwnership({
  tokenId, contractAddres, address
}: IvalidateOwnership) {
  try {
    const web3 = new Web3Service();

    if(!web3.providerETH) return false;

    const contract = new ethers.Contract(contractAddres, abiERC721, web3.providerETH);
